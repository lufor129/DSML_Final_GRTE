...
def rematch(text, tokens):
    ...
    for i, ch in enumerate(text):
        ch = ''.join([
            c for c in ch
            if not (ord(c) == 0 or ord(c) == 0xfffd or unicodedata.category(c) in ('Cc', 'Cf'))
        ])
        normalized_text += ch
    ...
    for token in tokens:
        if bool(token) and (token[0] == '[') and (token[-1] == ']'):
...
def mat_padding(inputs, length=None, padding=0):
    ...
    for x in inputs:
        if(length %2 == 0):
            ...
            pad_width[0] = (0, length+1 - x.shape[0])
            pad_width[1] = (0, length+1 - x.shape[0])            
...
def tuple_mat_padding(inputs,dim=1, length=None, padding=0):
    ...
    for x in inputs:
        if(length %2 == 0):
            ...
            pad_width[1] = (0, length+1 - x.shape[dim])
            pad_width[2] = (0, length+1 - x.shape[dim])
        x = np.pad(x, pad_width, 'constant', constant_values=padding)
        outputs.append(x)
    return np.array(outputs)
...
def sequence_padding(inputs,dim=0, length=None, padding=0):
    ...
    for x in inputs:
        if(length %2 == 0):
            ...
            pad_width[dim] = (0, length+1 - x.shape[dim])
        x = np.pad(x, pad_width, 'constant', constant_values=padding)
        outputs.append(x)
    return np.array(outputs)
